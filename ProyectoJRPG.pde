  ...
  if ( gameState == "START"){
   startGame();
    ...
    playGame();
      ...
      text("oops, algo no esta bien.",50,50);
  }
   }
   ...
   void startGame(){
     background(10);
    fill(255);
    text("Haz Click para empezar",140,200);
    text("Eres el ultimo hombre de armas debes resistir",60,260);
    text("las emboscadas de los enemigos escondidos",60,290);
...
class Chara implements Drawable {
  ...
  void drawShape() {
    pushMatrix();
      ...
      noStroke();
      fill(headColor);
      ellipse(0, 0, ChipSize, ChipSize / 2);
      fill(bodyColor);
      ellipse(0, ChipSize / 2, ChipSize, ChipSize / 2);
    popMatrix();
  }

  void drawHp(float x, float y, float w, float h) {
    fill(255, 0, 0); rect(x, y, w, h); 
    fill(0, 255, 0); rect(x, y, hp / (float)maxHp * w, h); 
  }
  void drawMagic(float x, float y, float w, float h) {
    fill(0, 0, 255); rect(y, x, w, h); 
    fill(0, 0, 255); rect(y, x, hp / (float)maxMagic * w, h); }

  void damage(int power) {
    hp = max(hp - power, 0);
    shake = 0.30f;
  }
  void magicDamage(int power){
    hp = max(hp - power, 0);
    shake = 0.15f;
  }
...
class PlayerChara extends Chara {
  PlayerChara() {
    super(icx(width / 2), icy(height / 2), color(255, 60, 55), color(0, 90, 255));
  }
...
class Battle implements Drawable {
  ...
  Button magicButton = new Button ("Hechizo", h, x - (margin - 1) * 0, w, margin);
